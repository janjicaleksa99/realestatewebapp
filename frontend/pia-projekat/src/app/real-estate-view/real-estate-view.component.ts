...
export class RealEstateViewComponent implements OnInit {
  ...
  edit() {
    if (this.realEstateForm.invalid) {
      ...
      let edit = true;
      if (this.ad.LastEdited != "0") {
        let date1 = new Date(this.ad.LastEdited);
        let date2 = new Date();
        if (date2.valueOf() - date1.valueOf() < 60 * 60 * 1000)
          edit = false;
      }
      if (edit) {
        this.ad.LastEdited = new Date().toString();
        if (this.salesman.type == 'agent') {
          this.agservice.getUserAgency(this.salesman.agency).subscribe((agency : Agency) => {
            ...
            this.aservice.updateAdvert(this.ad).subscribe((resp) => {
        ...
        alert("Svaku nekretninu mozete izmeniti najviše jednom po satu!");
      }
    }
  }
  ...
  checked(type, index) {
    let charac = this.ad.Realestate.Characteristics.includes(type);
    if (charac == true) {
      this.characSelected[index] = true;
    }
    return charac;
  }
}
